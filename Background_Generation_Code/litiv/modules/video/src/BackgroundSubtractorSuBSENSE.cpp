...
void BackgroundSubtractorSuBSENSE::apply(cv::InputArray _image, cv::OutputArray _fgmask, double learningRateOverride) {
    ...
    cv::Mat FluxTensor;
    FluxTensor.create(m_oImgSize,CV_8UC1);
    _fgmask.copyTo(FluxTensor);
    ...
    cv::Mat Diff_Tensor_Subsense;
    Diff_Tensor_Subsense.create(m_oImgSize,CV_8UC1);
    memset(Diff_Tensor_Subsense.data,0,oCurrFGMask.cols*oCurrFGMask.rows);
    ...
    if(m_nImgChannels==1) {
        ...
        for(size_t nModelIter=0; nModelIter<m_nTotRelevantPxCount; ++nModelIter) {
            ...
            *pfCurrMeanLastDist = (*pfCurrMeanLastDist)*(1.0f-fRollAvgFactor_ST) + fNormalizedLastDist*fRollAvgFactor_ST;
            ...
            // ==? good place to insert ghost detector?
...
//                if(FluxTensor_apply_count>12) {
//                    uchar *pDiff = Diff_Tensor_Subsense.ptr<uchar>(nCurrImgCoord_Y);
//                    uchar *pFlux = FluxTensor.ptr<uchar>(nCurrImgCoord_Y);
//
//                    if (pFlux[nCurrImgCoord_X] == 0) {
//                        pDiff[nCurrImgCoord_X] = UCHAR_MAX;
//                    }
//                }
            ...
            } // End of foreground and background checking
    ...
    // Post Processing
//    cv::imshow("Fluxtensor and subsense difference", Diff_Tensor_Subsense);
//    cv::imshow("Fluxtensor output123", FluxTensor);
